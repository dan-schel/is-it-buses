...
describe("LineGroupBuilder", () => {
  it("works", () => {
    const group = new LineGroupBuilder(400)
      .add(1)
      .add(2)
      .split()
      .add(3)
  ...
  it("throws if you terminate() too many times", () => {
    expect(() => {
      new LineGroupBuilder(400).add(1).terminate(97).terminate(98);
    }).toThrow();
  });

  it("throws if you attempt to add() to a terminated branch", () => {
    expect(() => {
      new LineGroupBuilder(400).add(1).terminate(97).add(2);
    }).toThrow();
  });

  it("throws if you attempt to split() once every branch is terminated", () => {
    expect(() => {
      new LineGroupBuilder(400).add(1).terminate(97).split();
    }).toThrow();
  });

  it("throws if you attempt to build() before every branch is terminated", () => {
    expect(() => {
      new LineGroupBuilder(400).add(1).build([]);
    }).toThrow();
  });
});
